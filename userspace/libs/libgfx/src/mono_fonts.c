...
static uint8_t *_get_font_glyph(gfx_font_t *font, char c)
{
    return (uint8_t *) font->data.monospace_atlas
           + (unsigned char) c * font->params.monospace.size.height;
}
    ...
    gfx_context_t *ctx, gfx_font_t *font, gfx_pos_t pos, gfx_color_t color, char c)
{
    ...
    for (int y = 0; y < font->params.monospace.size.height; y++) {
        uint8_t bits = glyph[y];
        for (int x = 0; x < font->params.monospace.size.width; x++) {
            if (bits & (0x80 >> x))
...
int _monospace_get_char_width(gfx_font_t *font)
{
